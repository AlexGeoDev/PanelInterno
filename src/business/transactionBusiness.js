...
function uploadFile(file) {
  ...
  let req = new Promise((ok) => {
    fetch(`${BASE_URL}fileupload`, { // Your POST endpoint
      method: 'POST',
      headers: {
...
function syncUsers(file) {
  ...
  let req = new Promise((ok) => {
    fetch(`${BASE_URL}api/v1/syncusers`, { // Your POST endpoint
      method: 'POST',      
      headers: {
        "Content-Type": "application/json"
      }
...
export { uploadFile, ID, listUsersSegment, identify, sendCredentials, trackEvent, processFile, syncUsers };